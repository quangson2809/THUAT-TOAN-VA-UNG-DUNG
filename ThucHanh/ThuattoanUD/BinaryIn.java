...
public final class BinaryIn {
    ...
    /**
     * Reads the next 16 bits from this binary input stream and return as a Unicode char (UTF-16).
     *
     * @return the next 16 bits as a {@code char}
     * @throws NoSuchElementException if there are fewer than 16 bits available
     */
    public char readCharUTF16() {
        if (isEmpty()) throw new NoSuchElementException("Reading from empty input stream");

        int hi = readChar();  // đọc byte cao
        if (isEmpty()) throw new NoSuchElementException("Reading from empty input stream");
        int lo = readChar();  // đọc byte thấp

        return (char) ((hi << 8) | lo);
    }

    /**
     * Reads the remaining bytes of data from this binary input stream and return as a UTF-16 string.
     *
     * @return the remaining data from this binary input stream as a {@code String}
     * @throws NoSuchElementException if this binary input stream is empty
     */
