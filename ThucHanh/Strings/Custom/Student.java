...
public class Student implements Comparable<Student> {
    ...
    public TST<Double> bangdiem(){return this.bangdiem;}

    /** tra ve chuoi bieu dien thong tin sinh vien*/
    @Override
    public String toString(){
        
     return String.format("%-10s | %10s %10s | %10s | %10s | %8.2f |  " + this.bangdiemtoString()  ,maSv,hodem,ten,ngaysinh,que,diemTb);

    }

    String bangdiemtoString (){
        String str ="___";
        if(this.bangdiem().size() != 0){
            for(String key: this.bangdiem().keys()){
                str += " | " + key+":"+ this.bangdiem().get(key).toString() + " ";
            }
        }
        return str;
    }
  ...
  public static void main(String[] args) {
    ...
    Arrays.sort(a, new maSvOrder());
    for(Student c: a) {
        StdOut.println(c);
    }
    
    StdOut.println("theo ten :");
    Arrays.sort(a, new tenOrder());
    for(Student c: a) {
        StdOut.println(c);
    }
