...
public class BST<Key extends Comparable<Key>, Value> {
  ...
  /*************************************************************************
    ...
    ***************************************************************************/
    ...
    private Queue<Node> Search (Key lo, Key hi){
        return Search(root, lo,hi);
    }
    private  Queue<Node> Search (Node root , Key lo, Key hi){
        ...
        int locmp = root.key.compareTo(lo);
        int hicmp = root.key.compareTo(hi);

        if( locmp >= 0 && hicmp <= 0)
            queue.enqueue(root);

        if( locmp > 0)
            q1 = Search(root.left,lo,hi);

        if( hicmp < 0 )
            q2 = Search(root.right, lo, hi);

        for (Node c: q1) queue.enqueue(c);
        for (Node c :q2) queue.enqueue(c);
        return queue;
    }

    public Queue<Key> rangSearch(Key lo, Key hi){
        Queue<Key> keys = new Queue<>();
        Queue<Node> queue = Search(lo,hi);
        for(Node c : queue){
            keys.enqueue(c.key);
        }
        return keys;
    ...
    public static void main(String[] args)throws IOException { 
        ...
        for (String s: st.rangSearch("A","K" ))
            StdOut.println(s + "-");
    }
}
